...
def index(request):
    if request.user.is_authenticated:
        ...
        image_to_be_displayed = EventTable.fetch('gravityspy',
                                                  ...
                                                  columns=['url1', 'url2', 'url3', 'url4', 'gravityspy_id', 'ml_label'], db='gravityspytools', passwd=require_env('GRAVITYSPYTOOLS_DATABASE_PASSWD', legacy=['GRAVITYSPYTOOLS_PASSWORD']), user=require_env('GRAVITYSPYTOOLS_DATABASE_USER', legacy=['GRAVITYSPYTOOLS_USER']), host=science_db_host())
