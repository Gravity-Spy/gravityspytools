...
from .science_tables import validate_science_table
...
def similarity_search(form):
    ...
    engine = create_engine('postgresql://{0}:{1}@{2}:5432/gravityspy'.format(os.environ['GRAVITYSPY_DATABASE_USER'], os.environ['GRAVITYSPY_DATABASE_PASSWD'], science_db_host()))
    if zooID:
    ...
    glitches = EventTable.fetch('gravityspy', 'glitches_v2d0 WHERE "gravityspy_id" IN (\'{0}\')'.format(str("','".join(list(SI['gravityspy_id'])))), columns = ['gravityspy_id', 'url1', 'url2', 'url3', 'url4', 'ifo', 'links_subjects', 'snr', 'peak_frequency', 'duration', 'central_freq', 'bandwidth', 'ml_label', 'event_time', 'Filename1', 'Filename2', 'Filename3', 'Filename4', 'peak_time'], host=science_db_host())
    ...
    if zooID:
        ...
        SI_glitches['searchedID'] = SI_glitches.loc[SI_glitches.event_time == gpstime, 'gravityspy_id'].iloc[0]
    else:
    ...
    if gravityspy_id:
        ...
        SI_glitches['searchedzooID'] = SI_glitches.loc[SI_glitches.event_time == gpstime, 'links_subjects'].iloc[0]
    else:
