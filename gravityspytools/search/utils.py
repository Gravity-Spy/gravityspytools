...
def similarity_search(form):
    ...
    if zooID:
        ...
        searched_image = features[numpy.argwhere(SI['uniqueID'].values == uniqueID)][0][0]

    distances = []
    for feature in features:
        distances.append(1-distance.cosine(searched_image,feature))

    tmp = pd.DataFrame({'distances' : distances, 'uniqueID' : SI['uniqueID'].values})
